<template>
  <b-card class="search-guarantees" :header-class="collapsed ? 'p-0' : 'rounded-bottom p-0 bg-white'" no-body>
    <collapse-button slot="header" :title="$t('search.guarantee')" :collapsed="collapsed" @toggle="toggleCollapse"/>
    
    <b-card-body v-show="collapsed" class="p-0">
      <b-form-group class="mb-0 p-3">
        <b-input-group>
          <b-form-input :placeholder="`${$t('search.guaranteeNameSearch')}...`" v-model="searchKey"/>
        </b-input-group>
      </b-form-group>
...
export default {
  ...
  computed: {
    value () {
      ...
      return args ? args.guaranteeIds : null
    },
    filteredItems () {
  ...
  methods: {
    ...
    submit () {
      ...
      if (this.selectedItems && this.selectedItems.length)
        query.guarantees = this.selectedItems.join()
      else
        delete query.guarantees
