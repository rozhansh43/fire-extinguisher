<template>
  <div>
    ...
    <div v-if="countdown > 0" class="text-primary text-center">
      <span class="text-muted">{{ $t('account.resendCode') }}</span>
      ({{ countdown }})
    </div>

    <b-btn v-else :disabled="progressing" variant="link" class="p-0" block @click="emitResendOtp">
  ...
  methods: {
    ...
    emitVerifyOtp () {
      this.$validator.validateAll().then(isValid => {
        if (isValid) {
          this.$emit('verifyOtp', this.verificationCode)
        }
      })
    }
  }
