<template>
  <b-card v-if="query && query.credit && (query.err || query.ref || query.rec)" body-class="rounded overflow-hidden p-4">
    <b-row>
      ...
      <b-col>
        <h4 class="mb-4" :class="query.err ? 'text-danger' : 'text-success'">
          ...
          <template v-else>
            {{ $t('credit.creditIncreaseWasSuccessful') }}.
          </template>
        </h4>
        ...
        <b-btn
          ...
          variant="info"
          class="px-4"
          :href="`${query.protocol}://${$config.websiteUrl.split('//')[1]}/dash/credit`"
        >
